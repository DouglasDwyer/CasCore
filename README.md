...
The `WithDefaultSandbox` method should provide a sensible default whitelist that ensures any loaded assemblies cannot gain access to the host system. Testing reveals that `Newtonsoft.Json` is able to run with the default sandbox policy, so it should be fairly comprehensive. (On modern target frameworks, `Newtonsoft.Json` checks [`RuntimeFeature.IsDynamicCodeSupported`](https://learn.microsoft.com/en-us/dotnet/api/system.runtime.compilerservices.runtimefeature.isdynamiccodesupported) to decide whether to generate accessors with `System.Reflection.Emit`; `CasCore` shims this property to report `false` for sandboxed assemblies, so `Newtonsoft.Json` falls back to plain reflection instead.)
